...
function grad_p_function_loop, Ne0, Te0
  ...
  for iNe=0,NNe-1 do begin
     for iTe=0,NTe-1 do begin
        ...
        grad(iTe,iNe,0) = p_value * (1./sigNe)*(expN  - q*expT)                             ; \propto dP/dNm
        grad(iTe,iNe,1) = p_value * (1./sigTe)*(expT  - q*expN)                             ; \propto dP/dTm
        grad(iTe,iNe,2) = p_value * (1./sigTe)*(expT2 - q*expTN - (1.-q^2))                 ; \propto dP/sigT
        grad(iTe,iNe,3) = p_value * (1./sigNe)*(expN2 - q*expTN - (1.-q^2))                 ; \propto dP/sigN
        grad(iTe,iNe,4) = p_value * (q + expTN - (q/(1.-q^2))*(expN2 + expT2 - 2.*q*expTN)) ; \propto dP/dq
     ENDFOR
  ENDFOR
  return, (1./(1-q^2))*grad
end
